...
import com.editmypdffree.rdtl.model.TextToPDFOptions;
import com.editmypdffree.rdtl.util.Constants;
import com.editmypdffree.rdtl.util.DialogUtils;
import com.editmypdffree.rdtl.util.DirectoryUtils;
...
import com.editmypdffree.rdtl.util.TextToPDFUtils;
import com.editmypdffree.rdtl.util.TextToPdfAsync;
...
public class TextToPdfFragment extends Fragment implements OnItemClickListener,
    ...
    @OnClick(R.id.selectFile2)
    public void selectTextFile2() {
        if (!mPermissionGranted) {
            getRuntimePermissions();
            return;
        }
        ...
        if (requestCode == mFileSelectCode) {
            if (resultCode == RESULT_OK) {
               ...
               // StringUtils.getInstance().showSnackbar(mActivity, R.string.text_file_selected);
                ...
                mSelectFile2.setText(getString(R.string.text_file_name) + fileName);
                mCreateTextPdf.setEnabled(true);
                mMorphButtonUtility.morphToSquare(mCreateTextPdf, mMorphButtonUtility.integer());

                if (mCreateTextPdf.getVisibility() == View.GONE)
                {
                    mCreateTextPdf.setVisibility(View.VISIBLE);
                }

                if (mTextEnhancementOptionsRecycleView.getVisibility() == View.GONE)
                {
                    mTextEnhancementOptionsRecycleView.setVisibility(View.VISIBLE);
                }
